...
class FakePublisher:
    ...
    def record_completion(self, pool_id, group_id, *, failed, error=None):
        assert pool_id == self.pool_id
        self.completions.append((group_id, failed, error))
...
def test_pool_publishes_failure_summary():
    ...
    assert pool.do() == [None]
    assert publisher.completions == [("broken", True, "ValueError: boom")]

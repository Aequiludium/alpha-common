...
`ygo top` 使用共享内存读取聚合状态，只更新发生变化的表格单元格。它显示所有
实时任务组和最近完成的 100 个任务组；完成记录保存在平台用户状态目录中，不会
随生产进程退出而清理。`STARTED` 使用本机时间，默认让最新任务排在顶端。点击
任意列头可按该字段排序，再次点击可切换升序和降序。业务日志仍保留在原程序
终端，不会破坏监控界面。
